...
class Point {
    ...
    getPointToCurveProjection(InPoint){
        let point1=this.getXY();
        // 斜率m
        let m=kernel_9(point1,InPoint)
        // 计算斜率倒数——连线的垂直斜率
        let m_vertical=-Math.pow(m,-1);
        // 对于曲线上的每个点 (xi, yi)，计算点P到该点连线的垂直投影的坐标。
        // 截距
        let b=point1.y-m_vertical*point1.x;
        // 投影点
        Px_proj=(InPoint.x+m_vertical*(InPoint.y-b))/(1+Math.pow(m_vertical,2))
        Py_proj=m_vertical*Px_proj+b;
        return [Px_proj,Py_proj];    
    }
    /**
    ...
    getDistancePointToCurve(Inpointset){
        let point=new Point(this.x,this.y);
        for(let i=0;i<Inpointset.length;i++){
            point.getEuclideanDistance_(Inpointset[i])
        }
